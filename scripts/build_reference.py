...
<div class="grid cols-2 tops">
  <div class="card">
    <h3 style="margin-top:0">Membrane</h3>
    <p style="color:var(--ink-2)">One cell's ECS-facing skin, in the crop's own frame. The cell
    with the most ECS-facing surface is meshed at 16 nm; the patch is the part of it that faces
    extracellular space.</p>
    <p style="color:var(--ink-2)">Coloured by signed curvature, protrusion / indentation against
    a 60 nm smoothed reference, or the gap to the nearest neighbouring cell.</p>
    <p class="note" style="margin:0">Faces at the crop wall are trimmed: marching cubes caps the
    volume there, and the cap is not membrane.</p>
  </div>
  <div class="card">
    <h3 style="margin-top:0">ECS</h3>
    <p style="color:var(--ink-2)">The space itself, meshed at 8 nm from an 800 nm cube of every
    crop &mdash; equal volume, so two crops can be compared on a shared camera. The cube is not
    the middle of the crop: it is the position whose ECS fraction is closest to the whole crop's,
    because the middle of a liver crop can sit inside one hepatocyte and hold no ECS at all.</p>
    <p style="color:var(--ink-2)">Coloured by curvature, protrusion / indentation, thickness or
    width. <b>Thickness</b> is the chord &mdash; march the normal through the space to the far
    wall. <b>Width</b> is the largest ball that fits, which the nearest wall in any direction
    bounds, so it stays smaller where sheets meet.</p>
    <p class="note" style="margin:0">The cut faces are kept, in a darker grey, so the space reads
    as the solid it is. Curvature is averaged over 24 nm before it is drawn.
    <a href="membranes/methods.html">The methods in full</a> has the rest.</p>
  </div>
</div>
<p class="note">Both surfaces use one sign convention: positive curvature and positive deviation
mean the membrane bulges <b>into</b> the extracellular space. Where a reading cannot be trusted
&mdash; a kernel that reached the crop wall, a ray that left the box &mdash; the vertex is grey
rather than wrong, and at 8 nm the narrowest resolvable channel is 16 nm, so the low end of the
thickness scale is the sampling and not the tissue.</p>
